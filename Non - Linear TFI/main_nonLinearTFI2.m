...
Qtotb  = xlsread('Qtot.xlsx','A1:U80'); % Q in boundary
Qtot   = xlsread('Qtot.xlsx','A1:U480'); % Q in domain and boundary
...
while (errorpsi>(1e-5) && errorlambda >(1e-5))
	...
	%determine new alpha
    lambda2 = lambda1*(15e6*1.257e-6)/(IT*q);
    alpha2=rlzm\(lambda2*ji);
    %find new lambda
