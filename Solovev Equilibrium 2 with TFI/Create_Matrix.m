...
%% initiation
Level = 4;     %level of expansion current profile
		 ...
		 for M=0:Level          %M
			 if((L+M)<=Level)
				 ...
				 for j=1:n   %element boundary
					 P=findQ3(rm(m),zm(m),nr(j),nz(j),lm(j),rb(j),zb(j),rb(j+1),zb(j+1),L,M,0,0.544,1.32);
					 Ptot=Ptot+P;
				 end
				 Q(m,O)=Q2(rm(m),zm(m),L,M,0,0.544,1.32,0.5)-Ptot; %form matriks Q for boundary area
				 Qtot(m,O)=Q(m,O);
			 end
		 end
	 end
         ...
         for L=0:Level
             for M=0:Level
                 if((L+M)<=Level)
                     ...
                     for j=1:n
                         P=findQ3(x(i),y(i),nr(j),nz(j),lm(j),rb(j),zb(j),rb(j+1),zb(j+1),L,M,0,0.544,1.32);
                         Ptot=Ptot+P;
                     end
                     Qtot(i+n,O)=Q2(x(i),y(i),L,M,0,0.544,1.32,1)-Ptot;
                 end
             end
         end 
		 fprintf('loop : %d\n',i);
